...
public class BaseParpareTestPc {
	//输出本页面日志 初始化
	static Logger logger = Logger.getLogger(BaseParpareTestPc.class.getName());
	protected SeleniumUtil seleniumUtil = null;
	//添加成员变量来获取beforeClass传入的context参数
	protected ITestContext testContext = null;
	protected static String testPcUrl = "";
	...
	public void startTestPcUrl(ITestContext context){
		...
		testPcUrl = context.getCurrentXmlTest().getParameter("testPcUrl");
		method = "/user/AdminLoginInterface.htm";
		
		try{
			//启动浏览器，打开浏览器，输入测试地址，并打开400*700的窗口
//			seleniumUtil.launchBrowserPc(browserName, context, testwwwUrl, timeOut);
			//启动浏览器，打开浏览器，输入测试地址，并最大化窗口
			seleniumUtil.launchBrowserPc(browserName, context, testPcUrl, method,timeOut);
		}catch(Exception e){
